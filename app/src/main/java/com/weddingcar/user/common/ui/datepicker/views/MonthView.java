package com.weddingcar.user.common.ui.datepicker.views;
...
import com.weddingcar.user.common.ui.datepicker.bizs.calendars.DPCManager;
import com.weddingcar.user.common.ui.datepicker.bizs.decors.DPDecor;
import com.weddingcar.user.common.ui.datepicker.bizs.themes.DPTManager;
import com.weddingcar.user.common.ui.datepicker.cons.DPMode;
import com.weddingcar.user.common.ui.datepicker.entities.DPInfo;
...
public class MonthView extends View {
    ...
    private static final HashMap<String, Set<String>> selectedMap = new HashMap<>();
    ...
    private void drawDecor(Canvas canvas, Rect rect, DPInfo info) {
        if (!TextUtils.isEmpty(info.strG)) {
            ...
            if (null != mDPDecor && info.isDecorTR) {
                canvas.save();
                mDPDecor.drawDecorTR(canvas, rect, mPaint,
                        centerYear + "-" + centerMonth + "-" + info.strG);
                canvas.restore();
            }
    ...
    void setSelectedDate(List<String> dateList) {
        dateSelected = dateList;
//        setMap(dateSelected, selectedMap);
//        prepareSelectedRegion();
        invalidate();
    }

    private void setMap(List<String> date, HashMap<String, Set<String>> cache) {
        cache.clear();
        for (String str : date) {
    ...
    void prepareSelectedRegion() {
        ...
        Set<String> map = selectedMap.get(centerYear + ":" + centerMonth);
        if (map == null) {
            map = new HashSet<>();
        }
        Region[][] tmp;
            ...
            for (int j = 0; j < tmp[i].length; j++) {
                ...
                if (region.contains(x, y)) {
                    ...
                    if (mDPMode == DPMode.SINGLE) {
                        ...
                        final String date = centerYear + "-" + centerMonth + "-" +
                                mCManager.obtainDPInfo(centerYear, centerMonth)[i][j].strG;
                        this.onDateClickListener.onClick(date);
