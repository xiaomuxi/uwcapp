...
import com.weddingcar.user.function.main.adapter.BrandChildrenListAdapter;
import com.weddingcar.user.function.main.adapter.CarBrandListAdapter;
...
public class CarBrandActivity extends BaseActivity implements View.OnClickListener{
    ...
    public void goBackAndBringResult() {
        String brandKey = carBrandsList.get(brandParentIndex).getKey()+"";
        String brandValue = carBrandsList.get(brandParentIndex).getValue();
        ...
        intent.putExtra("CAR_BRAND_ID", brandKey);
        intent.putExtra("CAR_BRAND_VALUE", brandValue);
