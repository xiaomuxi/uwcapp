...
public class CallingListAdapter extends MyBaseAdapter<OrderListEntity.Data>{

    public CallingListAdapter(Context c) {
        super(c);
    }
    ...
    protected View getView(int position, View convertView, ViewGroup parent, ViewHolder holder) {
        ...
        if (StringUtils.equals(item.getState(), "待支付")) {
            tv_enroll_car_count.setText(mContext.getResources().getString(R.string.text_choosed_car_count, item.getCount()+""));
        }
        else {
            tv_enroll_car_count.setText(mContext.getResources().getString(R.string.text_view_enroll_car_count, item.getCountSignUp()+""));
        }
